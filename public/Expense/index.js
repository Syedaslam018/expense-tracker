try {
  ...
  async function dailyFunc(e) {
    ...
    let dailyData = await axios.get(
      `http://localhost:3000/expense/daily-expenses`,
  ...
  async function monthlyFunc(e) {
    ...
    let monthlyData = await axios.get(
      `http://localhost:3000/expense/monthly-expenses`,
      { headers: { Authorization: token } }
    );
  ...
  async function yearlyFunc(e) {
    ...
    let yearlyData = await axios.get(
      `http://localhost:3000/expense/yearly-expenses`,
      { headers: { Authorization: token } }
    );
  ...
  limitChange = async () => {
    ...
    let token = localStorage.getItem("token");
    const pageLoad = await axios.get(
      `http://localhost:3000/expense/get-expense?page=1&limit=${localStorage.getItem(
        "limit"
      )}`,
      { headers: { Authorization: token } }
    );
    ...
    button.onclick = async function (e) {
      try {
        ...
        const del = await axios.delete(
          `http://localhost:3000/expense/delete-expense/${obj._id}`,
          { headers: { Authorization: token } }
        );
      ...
      var options = {
        ...
        handler: async function (response) {
          ...
          const postData = await axios.post(
            "http://localhost:3000/purchase/paymentStatus",
            {
  ...
  leadButt.onclick = async () => {
    ...
    const getData = await axios.get(
      "http://localhost:3000/premium/showLeaderBorad",
      { headers: { Authorization: token } }
    );
  ...
  download.onclick = async () => {
    try {
      let token = localStorage.getItem("token");
      const resp = await axios.get("http://localhost:3000/user/download", {
        headers: { Authorization: token },
      });
      if (resp.status === 200) {
  ...
  oldData.onclick = async () => {
    ...
    const content = await axios.get("http://localhost:3000/user/oldFiles", {
      headers: { Authorization: token },
    });
  ...
  async function getExpenses(page) {
    ...
    const data = await axios.get(
      `http://localhost:3000/expense/get-expense?page=${page}&limit=${localStorage.getItem(
        "limit"
      )}`,
      { headers: { Authorization: token } }
    );
  ...
  function displayLeaderBoard(obj) {
    ...
    let li = document.createElement("li");
    li.innerHTML =
      obj.name + "---" + "total - expenses" + "---" + obj.totalExpenses;
    list.appendChild(li);
  }
} catch (error) {
  console.log(error);
